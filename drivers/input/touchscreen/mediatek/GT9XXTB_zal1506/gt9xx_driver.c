...
#if defined(FTS_GESTURE_SYSFS_MODE)
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	if ((!cfg_info_len[1]) && (!cfg_info_len[2]) && (!cfg_info_len[3]) && (!cfg_info_len[4]) && (!cfg_info_len[5])&& (!cfg_info_len[6])) 
	    	{
			sensor_id = 0;
		} 
	else {
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH) {
		//GTP_ERROR("CTP_CONFIG_GROUP%d is INVALID CONFIG GROUP!",sensor_id + 1);
		pnl_init_error = 1;
		return -1;
	}
#if defined(CONFIG_GTP_COMPATIBLE_MODE)
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_hq_ctp_module_fw_version = grp_cfg_version;  //add by wangshouping
	...
	__set_bit(KEY_GESTURE_C, tpd->dev->keybit);  //liguangxian 20140913
	__set_bit(KEY_GESTURE_V, tpd->dev->keybit);
	__set_bit(KEY_GESTURE_M, tpd->dev->keybit);
	__set_bit(KEY_GESTURE_O, tpd->dev->keybit);

	__set_bit(KEY_GESTURE_W, tpd->dev->keybit);  //liguangxian 20140913
	...
	/* read pid */
	ret = gtp_i2c_read_dbl_check(client,0x8140, &buf[GTP_ADDR_LENGTH], 6);
	if (FAIL == ret) {
		GTP_ERROR("[get_ic_fw_msg]get pid & vid failed,exit");
		return FAIL;
	}
	fw_version = buf[GTP_ADDR_LENGTH + 4] + (buf[GTP_ADDR_LENGTH + 5] << 8);
	sprintf(g_tpd_module_name,"Lide_%x",fw_version);//wangshouping for ctp moudule
		...
		if (DOZE_ENABLED == doze_status) {
			...
			if(doze_buf[2] == 'c')
				...
				GTP_DEBUG("aaatpd->gesture_enablec=c");//nyadd
	                    	input_report_key(tpd->dev, KEY_GESTURE_C, 1);   // lgx0913
	                    	input_sync(tpd->dev);
                		...
                		{       
				GTP_DEBUG("aaatpd->gesture_enablec=o");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_O, 1);   // lgx0913
	                    	input_sync(tpd->dev);
                		...
                		{            
				GTP_DEBUG("aaatpd->gesture_enablec=w");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_W, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=e");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_E, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=v");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_V, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=m");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_M, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=s");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_S, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=z");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_Z, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=right");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_RIGHT, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=left");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_LEFT, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=down");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_DOWN, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				{               
				GTP_DEBUG("aaatpd->gesture_enablec=up");//nyadd
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE_UP, 1);   // lgx0913
	                    	input_sync(tpd->dev);
				...
				//doze_status = DOZE_WAKEUP;
	                    	input_report_key(tpd->dev, KEY_GESTURE, 1);   // lgx0913
	                    	input_sync(tpd->dev);
