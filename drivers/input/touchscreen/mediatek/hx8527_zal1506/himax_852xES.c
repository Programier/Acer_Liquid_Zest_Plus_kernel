...
extern int register_dotview_notifier(struct notifier_block *nb);
int value;
static int hallsensor_status=0;
static int hallsensor_status_flag=0;
//static int hall_state_ata;
//static u8 points_dotview = 0;
//static u8 points_dotview_counts=0;
#endif
...
extern u16 g_hq_ctp_module_fw_version;
	...
	static unsigned char i_CTPM_FW[]=
	{
		...
		//#include "HQ_AW875_HX8527E44_TXD_A.0E.06_20160107.i"
		  ...
		  #include "HQ_AL1506_HX8527E44_Booyi_A.0F.0D_20160606.i"
...
static void himax_read_TP_info(struct i2c_client *client)
{
	...
	sprintf(g_tpd_module_name,"Booyi");//wangshouping for ctp moudule
...
void dotview_test_release(struct kobject *kobject); 
ssize_t dotview_test_show(struct kobject *kobject, struct attribute *attr,char *buf);
ssize_t dotview_test_store(struct kobject *kobject,struct attribute *attr,const char *buf, size_t count);
...
//kobject对象的操作 
struct sysfs_ops dotview_test_sysops = 
{ 
        .show = dotview_test_show, 
        .store = dotview_test_store, 
}; 
 
/*定义kobject对象的一些属性及对应的操作*/ 
struct kobj_type ktype =  
{ 
        .release = dotview_test_release, 
        .sysfs_ops=&dotview_test_sysops, 
...
{ 
	int t;
	uint8_t cmd[2];
	int res = sscanf(buf, "%d", &value);
    	if (res != 1) 
    		{
        		I("fts_gesture_enable_store input string :'%s' \n",buf);
    		} 
    	else 
    		{
			I("[fts_gesture_enable_store] tpd->gesture_enable %d\n", value);
    		}
	...
	cmd[0]=0xf4;
        ...
        //printk("write: %s\n",buf);
         ...
         {
         	cmd[1]=0x40;
         	i2c_himax_master_write(private_ts->client, cmd, 2, DEFAULT_RETRY_CNT);	
         }
         else
         {
         	cmd[1]=0x00;
	...
	size_t len, loff_t *pos)
{
	...
	if(!HX_PROC_SEND_FLAG)
		{
			ret += sprintf(buf, "HX8527E44_Booyi.ver.0A.0F.%x\n", ts_data->vendor_config_ver);
...
#if defined(CONFIG_LEATHER_CASE)
register_dotview_notifier(&hallsensor_status_handler);
#endif
...
#ifdef HX_SMART_WAKEUP
	#if defined(CONFIG_LEATHER_CASE)
	if(ts->SMWP_enable && hallsensor_status==0)
	{
		hallsensor_status_flag=1;
	#else
	if(ts->SMWP_enable)
	{
	#endif
...
#ifdef HX_SMART_WAKEUP
	#if defined(CONFIG_LEATHER_CASE)
	if(ts->SMWP_enable && hallsensor_status_flag==1)
	{
		hallsensor_status_flag=0;
	#else
	if(ts->SMWP_enable)
	{
	#endif
...
static int __init himax852xes_init(void)
{
	#if defined(CONFIG_LEATHER_CASE)
	int res;
	res=kobject_init_and_add(&kobj,&ktype,NULL,"dotview_test");//初始化kobject对象kobj,并将其注册到linux系统
	#endif
