...
signed int force_get_current(void)
{
	...
	return (fg_current_state ? fg_current_temp  : -fg_current_temp) / 10;
}

int force_get_tbat(kal_bool update)
{
#ifdef CONFIG_TARGET_BUILD_NTCLOSS  //add wenchao
   // #if defined(CONFIG_POWER_EXT) || defined(FIXED_TBAT_25)
	bm_print(BM_LOG_CRTI, "[force_get_tbat] fixed TBAT=35 t\n");
	return 25;
#elif  defined(CONFIG_POWER_EXT) || defined(FIXED_TBAT_25)
	bm_print(BM_LOG_CRTI, "[force_get_tbat] fixed TBAT=25 t\n");
	printk("@@Vinton force_get_tbat\n");
	return 25;
#else
...
void dod_init(void)
{
  ...
  printk("fmx_get_boot_reason()=%d\n",get_boot_reason());
	...
	g_rtc_fg_soc = get_rtc_spare_fg_value();
#endif
   printk("fmx_abs_get_boot_reason()=%d\n",get_boot_reason());

   printk("fmx_abs(g_rtc_fg_soc - gFG_capacity_by_v))=%d\n",g_rtc_fg_soc - gFG_capacity_by_v);
   printk("fmx_abs_capacity_tolrance=%d\n",batt_meter_cust_data.cust_poweron_delta_capacity_tolrance);
   
   printk("fmx_abs(gFG_capacity_by_v_init - g_rtc_fg_soc)=%d\n",gFG_capacity_by_v_init - g_rtc_fg_soc);
   printk("fmx_abs(gFG_capacity_by_v - gFG_capacity_by_v_init)=%d\n",gFG_capacity_by_v - gFG_capacity_by_v_init);
#if defined(IS_BATTERY_REMOVE_BY_PMIC)
/*
	if (is_battery_remove_pmic() == 0 && (g_rtc_fg_soc != 0)
		&& batt_meter_cust_data.vbat_remove_detection) {
		printk("[fmx_dod_FGADC]is_battery_remove()==0 , use rtc_fg_soc%d\n",
			 g_rtc_fg_soc);
		gFG_capacity_by_v = g_rtc_fg_soc;
	} else {*/
...
if (((g_rtc_fg_soc != 0)
			...
			&& (get_boot_reason() == BR_WDT_BY_PASS_PWK || get_boot_reason() == BR_WDT
			    ...
			    || get_boot_reason() == BR_2SEC_REBOOT || get_boot_mode() == RECOVERY_BOOT)) && ((abs(g_rtc_fg_soc - gFG_capacity_by_v)) <=
		       batt_meter_cust_data.cust_poweron_delta_capacity_tolrance)))
#else
if (((g_rtc_fg_soc != 0)
		     &&
...
#endif
		{
		    printk("fmx_gFG_capacity_by_v=%d\n",gFG_capacity_by_v);
			printk("fmx_g_rtc_fg_soc=%d\n",g_rtc_fg_soc);
			gFG_capacity_by_v = g_rtc_fg_soc;
		} else {
...
void oam_run(void)
{
	...
	charging_current = battery_meter_get_charging_current();//Add for batt test
	printk("[Batt test]:  %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,%d\n", 
           oam_v_ocv_1,vol_bat,oam_i_1,charging_current,gFG_BATT_CAPACITY_aging, 100-oam_d_5,BMT_status.UI_SOC,oam_d_1,oam_d_2,oam_d_3,oam_d_4,oam_d_5);
}
...
{
    static signed int previous_gFG_capacity_by_c = 0;
	int i = 0;
	int ret = 0;
#ifdef MTK_BATTERY_LIFETIME_DATA_SUPPORT
	int columb_delta = 0;
...
/* 2. Calculate battery capacity by VBAT */
	gFG_capacity_by_v = fgauge_read_capacity_by_v(gFG_voltage);
  //*fmx add for SOC tiao bian 2016_5_3++++++++
  printk("fmx_gFG_capacity_by_c=%d,%d,%d\n",gFG_capacity_by_c,previous_gFG_capacity_by_c,fgauge_read_capacity(1));
/* 3. Calculate battery capacity by Coulomb Counter */
     gFG_capacity_by_c = fgauge_read_capacity(1);
     if(previous_gFG_capacity_by_c < gFG_capacity_by_c && (gFG_capacity_by_c - previous_gFG_capacity_by_c > 1)){
	               gFG_capacity_by_c = previous_gFG_capacity_by_c+1;
	          }else{
                  gFG_capacity_by_c = fgauge_read_capacity(1);
				  }
	
	if(previous_gFG_capacity_by_c > gFG_capacity_by_c && (previous_gFG_capacity_by_c - gFG_capacity_by_c > 1)){
	               gFG_capacity_by_c = previous_gFG_capacity_by_c-1;
	          }else{
                  gFG_capacity_by_c = fgauge_read_capacity(1);
				  }
	previous_gFG_capacity_by_c = gFG_capacity_by_c;
//*fmx add for SOC tiao bian 2016_5_3++++++++
/* 4. voltage mode */
...
/* 5. Logging */
	bm_print(BM_LOG_FULL,
		 ...
		 batt_meter_cust_data.car_tune_value, batt_meter_cust_data.aging_tuning_value);
			 ...
			 printk("[FMX_FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
			 gFG_Is_Charging, gFG_current, gFG_columb, gFG_voltage, gFG_capacity_by_v,
			 gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
			 gFG_BATT_CAPACITY_aging, gFG_compensate_value, gFG_ori_voltage,
			 batt_meter_cust_data.ocv_board_compesate, batt_meter_cust_data.r_fg_board_slope,
