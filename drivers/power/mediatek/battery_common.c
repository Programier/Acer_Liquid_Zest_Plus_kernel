...
//#define HQ_CHARGER_LED_GPIO (109)
...
#ifdef CONFIG_HQ_SET_CURRENT_SUPPORT
int g_phone_CC_value = 9;
#endif
...
extern signed int force_get_current(void);
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		...
		val->intval = get_bat_charging_current_level(); // Vinton Real-time Current
		#else
		  val->intval = battery_meter_get_charging_current();//fmx1/6 for AL875 gong_mo_current
		#endif
		break;
...
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	...
	if ((BMT_status.ZCV <= BMT_status.nPercent_ZCV)
		...
		resetBatteryMeter = KAL_TRUE;
			    ...
			    printk("[nPercent1] ZCV %d <= nPercent_ZCV %d, UI_SOC=%d., tracking UI_SOC=%d\n",
		...
		resetBatteryMeter = KAL_TRUE;
			    ...
			    printk("[nPercent2] ZCV %d > BMT_status.nPercent_ZCV %d and UI SOC=%d, then keep %d.\n",
			    BMT_status.ZCV, BMT_status.nPercent_ZCV, BMT_status.UI_SOC,
			    BMT_status.nPrecent_UI_SOC_check_point);
	} else {
		timer_counter = (batt_cust_data.npercent_tracking_time / BAT_TASK_PERIOD);
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		/* reduce after xxs */
#if 0
	if ((upmu_is_chr_det() == KAL_TRUE && BMT_status.ICharging > 100) || (bat_is_recharging_phase() == KAL_TRUE) )
	{
		battery_log(BAT_LOG_CRTI, "[FMX_Sync_Real] UI_SOC=%d, SOC=%d\n",
...
#if 0//!defined(CUST_CAPACITY_OCV2CV_TRANSFORM)
		BMT_status.UI_SOC = BMT_status.SOC;
#else
       printk("[fmx_Sync_Real] UI_SOC=%d, SOC=%d, counter = %d\n",BMT_status.UI_SOC, BMT_status.SOC, timer_counter);
...
int get_bat_charging_vbus_level(void)
{
	int charging_vbus;

	battery_charging_control(CHARGING_CMD_GET_VBUS, &charging_vbus);

	return charging_vbus;
}
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	//FMX2016/3+
#ifdef CONFIG_HQ_TEMP_PROTECT_SUPPORT
			if(BMT_status.temperature <0)
				BMT_status.bat_temp_state = TEMP_POS_LOW;
			else if (BMT_status.temperature >= 0 &&BMT_status.temperature <= 15 ) 
				BMT_status.bat_temp_state = TEMP_0_to_15;
			else if(BMT_status.temperature > 15 &&BMT_status.temperature <= 48 )
				BMT_status.bat_temp_state = TEMP_POS_NORMAL;
			else if(BMT_status.temperature > 48 &&BMT_status.temperature <= MAX_CHARGE_TEMPERATURE )
				BMT_status.bat_temp_state = TEMP_45_to_55;
			else if(BMT_status.temperature > MAX_CHARGE_TEMPERATURE)
				BMT_status.bat_temp_state = TEMP_POS_HIGH;
#endif
	//FMX2016/3-
		...
		if (BMT_status.charger_exist == KAL_TRUE) {
			if (previous_charger_exist == KAL_FALSE) {
				...
				if ((BMT_status.charger_type == STANDARD_CHARGER) ||
					...
					data = get_bat_charging_current_level();
...
void mt_battery_GetBatteryData(void)
{
	...
	"AvgVbat=(%d,%d),AvgI=(%d,%d),VChr=%d,AvgT=(%d,%d),SOC=(%d,%d),UI_SOC=%d,ZCV=%d bcct:%d:%d I:%d\n",
		...
		g_bcct_flag, get_usb_current_unlimited(), force_get_current());
	
		printk("[Batt test]:  %d, %d, %d, %d, %d, %d,%d, %d, %d, %d, %d\n", BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		BMT_status.charger_vol, BMT_status.temperature, temperature,
	    BMT_status.SOC, BMT_status.UI_SOC, BMT_status.ZCV,g_bcct_flag);
...
static void mt_battery_notify_TotalChargingTime_check(void)
{
	...
	if ((g_battery_thermal_throttling_flag == 2) || (g_battery_thermal_throttling_flag == 3)) {
	    ...
	    if ((BMT_status.total_charging_time >= BAD_BATTERY_CHARGING_TIME )&& (BMT_status.SOC !=100))
		{
		g_BatteryNotifyCode |= 0x0040;
		battery_log(BAT_LOG_CRTI, "[BATTERY] Bad Battery\n");//2016/3/4 875_ACER check bad battery
		}else{
		g_BatteryNotifyCode &= ~(0x0040);
	    }
		if (BMT_status.total_charging_time >= MAX_CHARGING_TIME)
...
static void mt_battery_notify_VBatTemp_check(void)
{
...
#ifdef CONFIG_HQ_TEMP_PROTECT_SUPPORT
		int charger_status ;
		charger_status = get_charger_detect_status();
		if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE  && charger_status != 0)	
		{
			g_BatteryNotifyCode |= 0x0002;
			battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) over temperature & charger is exist\n", BMT_status.temperature);
		}else{
			g_BatteryNotifyCode &= ~(0x0002);
		}
		if(BMT_status.temperature <= MIN_CHARGE_TEMPERATURE && charger_status != 0){
			g_BatteryNotifyCode |= 0x0020;
			battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) under temperature  & charger is exist\n", BMT_status.temperature);
		}else{
			g_BatteryNotifyCode &= ~(0x0020);
		}
		if((BMT_status.temperature <= MAX_CHARGE_TEMPERATURE_MINUS_X_DEGREE  && g_batt_temp_status == TEMP_POS_HIGH)
			||(BMT_status.temperature >= MIN_CHARGE_TEMPERATURE_PLUS_X_DEGREE && g_batt_temp_status == TEMP_POS_LOW))
		{
			g_BatteryNotifyCode |= 0x0080;
			battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Temperature down(%d),allow charging \n", BMT_status.temperature);
			printk("FMX_g_BatteryNotifyCode_1= %d",g_BatteryNotifyCode);
		    battery_log(BAT_LOG_CRTI,"FMX_g_BatteryNotifyCode_1= %d",g_BatteryNotifyCode);
		} else{
			g_BatteryNotifyCode &= ~(0x0080);
			printk("FMX_g_BatteryNotifyCode_1_1= %d",g_BatteryNotifyCode);
		  battery_log(BAT_LOG_CRTI,"FMX_g_BatteryNotifyCode_1_1= %d",g_BatteryNotifyCode);
		}
#else
	//-fanmingxing 2016/3
	...
	battery_log(BAT_LOG_FULL, "[BATTERY] BATTERY_NOTIFY_CASE_0002_VBATTEMP (%x)\n",
		    g_BatteryNotifyCode);
#endif//CONFIG_HQ_TEMP_PROTECT_SUPPORT
#endif
}
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ADC_CHANNEL_READ:
		...
		} else if (adc_in_data[0] == 2) {	/* I_Charging */
			adc_out_data[0] =
				get_bat_charging_current_level() * adc_in_data[1];
		} else if (adc_in_data[0] == 3) {	/* V_Charger */
			//adc_out_data[0] = battery_meter_get_charger_voltage() * adc_in_data[1];
			adc_out_data[0] = get_bat_charging_vbus_level() * adc_in_data[1];  //phb add for ata test at 2016.4.19
...
#ifdef CONFIG_HQ_RUNIN_SUPPORT
	{
		...
		else if(battery_in_data[0]==2)
		{
			...
			printk("[Runin][BATTERY] adc_cali_ioctl cmd is 2 and turn off chg!!\n\r");
		...
		#ifdef CONFIG_HQ_SET_CURRENT_SUPPORT
		else if((battery_in_data[0] == 8) || (battery_in_data[0] == 9)){
			g_phone_CC_value = battery_in_data[0];
			//pchr_turn_on_charging();
		}
		#endif
		else
...
#if 0
	// Vinton add for Charger led
	ret = gpio_request(HQ_CHARGER_LED_GPIO, "Charger_led_config");
    if(ret < 0){
	    printk("[@@Vinton] request charger led gpio fail!\n");
		gpio_free(HQ_CHARGER_LED_GPIO);
	}
#endif
