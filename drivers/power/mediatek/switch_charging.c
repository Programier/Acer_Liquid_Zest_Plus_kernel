...
#if 0//CONFIG_HQ_TEMP_PROTECT_SUPPORT
	if (BMT_status.bat_temp_state == TEMP_0_to_15)
	{
		if(BMT_status.bat_vol <4050){
			printk("xingxing_1!\n");
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY]BATT temp=%d,voltage=%d :Set limit currnet to %dmA\r\n",BMT_status.temperature,BMT_status.bat_vol,Q_MAX_POS_25*30);	 
			g_temp_CC_value = Q_MAX_POS_25*30;//CHARGE_CURRENT_750_00_MA;
			g_temp_input_CC_value = Q_MAX_POS_25*30;//CHARGE_CURRENT_750_00_MA;
		}
		else {
			printk("xingxing_2!!\n");
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY]BATT temp=%d,voltage=%d :Set limit currnet to 500mA\r\n",BMT_status.temperature,BMT_status.bat_vol);	 
			g_temp_CC_value = CHARGE_CURRENT_550_00_MA;
			g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
		}
	} 
	else	if (BMT_status.bat_temp_state == TEMP_45_to_55)
	{
...
//-FMX2016/3
			...
			if (BMT_status.bat_temp_state == TEMP_0_to_15 || BMT_status.bat_temp_state == TEMP_45_to_55)
			{
				battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY]BATT temp=%d,voltage=%d :Set limit currnet to %dmA\r\n",BMT_status.temperature,BMT_status.bat_vol,Q_MAX_POS_25*30);  
				g_temp_CC_value = CHARGE_CURRENT_800_00_MA;//CHARGE_CURRENT_750_00_MA;
				g_temp_input_CC_value = CHARGE_CURRENT_800_00_MA;//CHARGE_CURRENT_750_00_MA;
...
#endif
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
		...
		#ifdef CONFIG_HQ_SET_CURRENT_SUPPORT
			if(g_temp_CC_value > 65000 && ((mt_get_bl_brightness() > 0) || (g_phone_CC_value == 8))){
				g_temp_CC_value = 650000;
				g_temp_input_CC_value = 650000;
				printk("[@@Vinton] brightness = %d, g_phone_CC_value = %d",mt_get_bl_brightness(), g_phone_CC_value);
			}
		#endif
          ...
          #else
			if (batt_cust_data.high_battery_voltage_support)
				cv_voltage = BATTERY_VOLT_04_420000_V;//4480000
			else
				cv_voltage = BATTERY_VOLT_04_200000_V;
          #endif
