...
static int pm_enable_m_flag = 0;
static int pm_enable_o_flag = 0;
...
/*----------------------------------------------------------------------------*/
extern void pm_mag_enable_data(int handle, int enable);//phb modified for take 3s to resume 2016.06.01
static int akm09911_suspend(struct i2c_client *client, pm_message_t msg)
{
...
//phb modified for take 3s to resume 2016.06.01 start
		if (atomic_read(&m_flag) == 1) {
			printk("PHB: disable akm09911_m by suspend\n");
			pm_enable_m_flag = 1;
			pm_mag_enable_data(0, 0);
		}

		if (atomic_read(&o_flag) == 1) {
			printk("PHB: disable akm09911_o by suspend\n");
			pm_enable_o_flag = 1;
			pm_mag_enable_data(1, 0);
		}
//phb modified for take 3s to resume 2016.06.01 end
...
static int akm09911_resume(struct i2c_client *client)
{
	...
	MAG_FUN();
//phb modified for take 3s to resume 2016.06.01 start
	if (pm_enable_m_flag == 1) {
		printk("PHB: enable akm09911_m by resume\n");
		pm_enable_m_flag = 0;
		pm_mag_enable_data(0, 1);
	}

	if (pm_enable_o_flag == 1) {
		printk("PHB: enable akm09911_o by resume\n");
		pm_enable_o_flag = 0;
		pm_mag_enable_data(1, 1);
	}
//phb modified for take 3s to resume 2016.06.01 end
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	this_client->addr = hw->i2c_addr[0];
	/* Check connection */
	...
	g_msensor_name = AKM09911_DEV_NAME;
	return 0;
