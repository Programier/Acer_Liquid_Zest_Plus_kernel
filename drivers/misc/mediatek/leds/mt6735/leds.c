...
//static DEFINE_SPINLOCK(pgd_lock);
static DEFINE_MUTEX(pgd_lock);

static BLOCKING_NOTIFIER_HEAD(led_notifier_list);

/**
 *	fb_register_client - register a client notifier
 *	@nb: notifier block to callback on events
 */
int led_register_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&led_notifier_list, nb);
}
EXPORT_SYMBOL(led_register_client);

/**
 *	fb_unregister_client - unregister a client notifier
 *	@nb: notifier block to callback on events
 */
int led_unregister_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&led_notifier_list, nb);
}
EXPORT_SYMBOL(led_unregister_client);

/**
 * fb_notifier_call_chain - notify clients of fb_events
 *
 */
int led_notifier_call_chain(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&led_notifier_list, val, v);
}
EXPORT_SYMBOL_GPL(led_notifier_call_chain);
...
#define PMIC_PERIOD_NUM 5
/* 100 * period, ex: 0.01 Hz -> 0.01 * 100 = 1 */
int pmic_period_array[] = {600, 2250, 4250, 8250, 10250 };
//{ 250, 500, 1000, 1250, 1666, 2000, 2500, 10000 };
/* int pmic_freqsel_array[] = {99999, 9999, 4999, 1999, 999, 499, 199, 4, 0}; */
int pmic_freqsel_array[] = {299,1999, 4499, 8749, 9999 };
//{ 0, 4, 199, 499, 999, 1999, 1999, 1999 };
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
   ...
   printk("[@@Vinton]mt_mt65xx_led_set_cust  cust->mode= %d, level = %d\n", cust->mode, level);
	...
	if (led_data->level != level) {
		...
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
			...
			if(old_level!=0 && level==0)
				led_notifier_call_chain(0,"no_use");  //add by wsp
			else if(old_level==0 && level!=0)
				led_notifier_call_chain(1,"no_use");  //add by wsp
			else
				;
			LEDS_DEBUG
			    ("Set Backlight directly %d at time %lu, mapping level is %d\n",
