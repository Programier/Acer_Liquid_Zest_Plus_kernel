...
static int pm_gyro_enable_flag = 0; //phb modified for take 3s to resume 2016.06.01
/*----------------------------------------------------------------------------*/
...
static int LSM6DS3_gyro_WriteCalibration(struct i2c_client *client, int dat[LSM6DS3_GYRO_AXES_NUM])
{
	struct lsm6ds3_gyro_i2c_data *obj = i2c_get_clientdata(client);
	int err = 0;
	int cali[LSM6DS3_GYRO_AXES_NUM] = {0};
...
extern void pm_gyro_enable_data(int enable); //phb modified for take 3s to resume 2016.06.01
/*----------------------------------------------------------------------------*/
static int lsm6ds3_gyro_suspend(struct i2c_client *client, pm_message_t msg)
{
	struct lsm6ds3_gyro_i2c_data *obj = i2c_get_clientdata(client);
...
//phb modified for take 3s to resume 2016.06.01 start
		...
		if (enable_status == true) {
			printk("PHB: disable gyro by suspend\n");
			pm_gyro_enable_flag = 1;
			pm_gyro_enable_data(0);
		}
...
//phb modified for take 3s to resume 2016.06.01 start
	...
	if (pm_gyro_enable_flag == 1) {
		printk("PHB: enable gyro by resume\n");
		pm_gyro_enable_flag = 0;
		pm_gyro_enable_data(1);
	}
//phb modified for take 3s to resume 2016.06.01 end
...
/*lenovo-sw caoyi1 modify A+G sensro iic address 20150417 begin*/
	//client->addr = 0xD4>>1;
	client->addr = 0x6B;//modify for real iic addr
...
#endif
	g_Gyro_name = LSM6DS3_GYRO_DEV_NAME;
	GYRO_LOG("%s: OK\n", __func__);
	return 0;
