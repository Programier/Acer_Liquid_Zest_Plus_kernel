...
#define VOL_1000 1000000
		...
		if (On) {
				...
				printk("[@@Vinton][PowerON]pinSetIdx:%d, currSensorName:%s\n", pinSetIdx, currSensorName);

				if (currSensorName && ((0 == strcmp(currSensorName,SENSOR_DRVNAME_IMX258_MIPI_RAW_OFG_ZAL1506 ))
									||(0 == strcmp(currSensorName, SENSOR_DRVNAME_IMX258_MIPI_RAW_SY_ZAL1506)))) {

						printk("[@@Vinton]:[PowerON]IMX258, pinSetIdx = %d\n", pinSetIdx);
				...
				else if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW_HQ_ZAL1506, currSensorName))
										||(0 == strcmp(SENSOR_DRVNAME_OV5670_MIPI_RAW_QH_ZAL1506, currSensorName))))
				{
						//First Power Pin low and Reset Pin Low
						printk("[@@Vinton]:[PowerON]ov5670, pinSetIdx = %d\n", pinSetIdx);
		...
		printk("[@@Vinton][PowerOFF]pinSetIdx:%d currSensorName:%s\n", pinSetIdx, currSensorName);
		if (pinSetIdx == 0)
				ISP_MCLK1_EN(0);
		else if (pinSetIdx == 1)
				//ISP_MCLK2_EN(0);
		...
		{
				//Set Power Pin low and Reset Pin Low
				printk("[@@Vinton][Power OFF] OV5670 !\n");
				if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
						mtkcam_gpio_set(pinSetIdx, CAMPDN,
				...
				if (TRUE != _hwPowerDown(VRF18_1)) {
						PK_DBG("[CAMERA SENSOR] Fail to OFF core power (VCAM_D_SUB),power id = %d\n",VCAMD);
						goto _kdCISModulePowerOn_exit_;
				}
				/* VCAM_IO */
				if (TRUE != _hwPowerDown(VCAMIO)) 
