...
#include "imx258mipiraw_Sensor_ofg.h"

#define PFX "IMX258_ofg"
...
extern bool read_imx258_eeprom_ofg( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258_eeprom_SPC_ofg( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool ofg_selective_read_eeprom(u16 addr, BYTE* data);
extern int read_imx258_of_eeprom_mtk_fmt(void);
extern int imx258_of_eeprom_update_awb(void);
...
static void load_imx258_SPC_Data(void)
{
	...
	{
		if(!read_imx258_eeprom_SPC_ofg(0x0CD2,IMX258MIPI_SPC_Data,126))  // o-film customized
		{
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 if(read_cmos_sensor(0x0A01) == 0x01)
			 {
				 ...
				 printk("imx258 type = 0x%x(0x00=HDR,0x10=binning,0x30=No PDAF.\n)", imx258_type);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
            *sensor_id = return_sensor_id();
			printk("[@@Vinton] imx258 ofg sensor_id = 0x%x\n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);

				//WENDELL-20160323 added
				{
					u16 otpVidAddrLow = 0x10;
					BYTE vendorIdLow = 0x0;

					ofg_selective_read_eeprom(otpVidAddrLow, &vendorIdLow);
					printk("[WENDELL]-%s: sensor id: 0x%x, vendorIdLow: 0x%x\n", __func__, *sensor_id, vendorIdLow);

					if(0x7 == vendorIdLow) {
						*sensor_id = IMX258_SENSOR_ID_OFG;
						read_imx258_of_eeprom_mtk_fmt();
					
						return ERROR_NONE;	
					}
					else{	
						printk("[WENDELL]-%s: vendorIdLow: 0x%x, this is imx258 but error vendor id\n", __func__, vendorIdLow);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258_eeprom_ofg((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
UINT32 IMX258_MIPI_RAW_SensorInit_Ofg(PSENSOR_FUNCTION_STRUCT *pfFunc)
