...
//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
	...
	if(gpio_get_value(sub_camera_gpio_id_pin)){
		printk("[@@Vinton] qunhui sub_camera_gpio_id_pin = %d\n", gpio_get_value(sub_camera_gpio_id_pin));
		goto SENSOR_ID_FAIL;
	}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            printk("[@@Vinton] qunhui gpio_get_value = %d. sensor_id = 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            printk("[@@Vinton] qunhui gpio_get_value = %d.\n",gpio_get_value(sub_camera_gpio_id_pin));
