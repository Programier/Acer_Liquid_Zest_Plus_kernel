...
#include "imx258mipiraw_Sensor_sy.h"



#define PFX "IMX258_sy"
...
extern bool read_imx258_eeprom_sy( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258_eeprom_SPC_sy( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool sy_selective_read_eeprom(u16 addr, BYTE* data);
...
static void load_imx258_SPC_Data(void)
{
	...
	{
		if(!read_imx258_eeprom_SPC_sy(0x0F73,IMX258MIPI_SPC_Data,126))
		{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
            *sensor_id = return_sensor_id();
			printk("[@@Vinton] imx258 sy sensor_id = 0x%x\n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//WENDELL-20160323 added
				{
					...
					sy_selective_read_eeprom(otpVidAddr, &vendorId);
					printk("[WENDELL]-%s: sensor id: 0x%x, vendorIdLow: 0x%x\n", __func__, *sensor_id, vendorId);

					if(0x7 != vendorId) {
						*sensor_id = IMX258_SENSOR_ID_SY;
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258_eeprom_sy((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
UINT32 IMX258_MIPI_RAW_SensorInit_Sy(PSENSOR_FUNCTION_STRUCT *pfFunc)
