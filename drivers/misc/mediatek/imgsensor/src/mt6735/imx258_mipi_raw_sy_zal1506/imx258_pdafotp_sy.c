...
#define IMX258_EEPROM_READ_ID_SY  0xA0
...
bool sy_selective_read_eeprom(u16 addr, BYTE* data)
{
	...
	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, IMX258_EEPROM_READ_ID_SY)<0)
		return false;
    return true;
}
...
static bool _read_imx258_eeprom(u16 addr, BYTE* data, u32 size ){
	...
	int offset = addr;
	for(i = 0; i < size; i++) {
		if(!sy_selective_read_eeprom(offset, &data[i])){
			return false;
		}
...
bool read_imx258_eeprom_sy( u16 addr, BYTE* data, u32 size){
	addr = 0x0763;
	size = 1404;
...
bool read_imx258_eeprom_SPC_sy( u16 addr, BYTE* data, u32 size){
	//int i;
	addr = 0x0F73;//0x0F73;
	size = 126;
