...
void bq25890_pumpx_up(unsigned int val)
{
	...
	battery_log(BAT_LOG_CRTI, "bq25890_pumpx_up  val=%d, ret=%d.", val, ret);
/* Input current limit = 800 mA, changes after port detection*/
	bq25890_set_iinlim(0x10);  // bq25890_CON0
/* CC mode current = 2048 mA*/
	bq25890_set_ichg(0x1a);  //bq25890_CON4
	msleep(3000);
}
...
unsigned int bq25890_get_idpm_state(void)
{
	...
	ret = bq25890_read_interface((unsigned char) (bq25890_CON13),
				     (&val),
				     (unsigned char) (CONB_IDPM_STAT_MASK),
				     (unsigned char) (CONB_IDPM_STAT_SHIFT)
	    );
	return val;
}
...
void bq25890_dump_register(void)
{
	...
	printk("[bq25890] charging register status");
	bq25890_ADC_start(1);
	for (i = 0; i < bq25890_REG_NUM; i++) {
		bq25890_read_byte(i, &bq25890_reg[i]);
		printk("[bq25890 reg@][0x%x]=0x%x \n", i, bq25890_reg[i]);
	}
	bq25890_ADC_start(1);
	...
	battery_log(BAT_LOG_CRTI, "[PE+]BQ25896 Ichg=%d mA, Vbat =%d mV, ChrStat=%d, CHGEN=%d\n",
		    ichg * 50, vbat * 20 + 2304, chrg_state, chr_en);
