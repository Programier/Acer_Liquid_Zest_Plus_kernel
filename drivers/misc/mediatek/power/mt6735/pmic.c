...
unsigned int upmu_get_rgs_chrdet(void)
{
	...
	printk("[upmu_get_rgs_chrdet] CHRDET status = %d\n", val);
...
{
	printk("[fmx_chrdet_int_handler]CHRDET status = %d....\n",
	...
	if (!upmu_get_rgs_chrdet()) {
        ...
        printk("fmx_boot_mode=%d\n",boot_mode);
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		    || boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
			printk("[chrdet_int_handler] Unplug Charger/USB\n");
			mt_power_off();
		}
	}
#endif
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	pmic_set_register_value(PMIC_WDTRSTB_STATUS_CLR, 1);
        printk("[Kernel_PMIC_INIT_SETTING_V1] cassy...\n");//add ет\u017eі
       ret = pmic_config_interface(0xF48, 0x0, 0x1, 0);//fmx cut pmic over_charge_v protect
