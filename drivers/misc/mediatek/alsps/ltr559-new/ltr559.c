...
static int ltr559_ps_get_thres_low(void);
static int ltr559_ps_get_thres_high(void);
...
static ssize_t ltr559_show_status(struct device_driver *ddri, char *buf)
{
	...
	if(ltr559_obj->hw) {
		len += sprintf(buf+len, "CUST: %d, (%d %d)\n", 
			ltr559_obj->hw->i2c_num, ltr559_obj->hw->power_id, ltr559_obj->hw->power_vol);
	} else {
		len += sprintf(buf+len, "CUST: NULL\n");
	}

	len += sprintf(buf+len,  "Suspend: als %d ps %d\n", atomic_read(&ltr559_obj->als_suspend), atomic_read(&ltr559_obj->ps_suspend));
	
	len += sprintf(buf+len,  "Threshold: ps_thd_val_h = %d, ps_thd_val_l = %d\n", ltr559_ps_get_thres_high(), ltr559_ps_get_thres_low());
...
static ssize_t Huaqin_show_val(struct device_driver *ddri, char *buf)
{
	...
	if((ltr559_ps_read(ltr559_obj->client, &ltr559_obj->ps)) != 0) {
		APS_ERR("ltr559_read_ps fail\n");
		return -1;
	}*/
	
	count = sprintf(buf, "LTR559 PS val=%d\n", current_noise);
		
	return count;
}

/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(Huaqin_show_val, S_IWUSR | S_IRUGO, Huaqin_show_val, NULL);
...
static int ltr559_ps_get_thres_low(void)
{
	...
	databuf[0] = ltr559_i2c_read_reg(LTR559_PS_THRES_LOW_0);
	if (databuf[0] < 0) {
		APS_DBG("read ps_thres_low_0 fail\n");
		goto out;
	}
	databuf[1] = ltr559_i2c_read_reg(LTR559_PS_THRES_LOW_1);
	if (databuf[1] < 0) {
		APS_DBG("read ps_thres_low_1 fail\n");
		goto out;
	}
	
	ps_thres_low = databuf[0] + ((databuf[1] & 0x0F) * 256);

	APS_DBG("ps_threshold_low = %d\n", ps_thres_low);
...
static int ltr559_ps_get_thres_high(void)
{
	...
	databuf[0] = ltr559_i2c_read_reg(LTR559_PS_THRES_UP_0);
	if (databuf[0] < 0) {
		APS_DBG("read ps_thres_up_0 fail\n");
		goto out;
	}
	databuf[1] = ltr559_i2c_read_reg(LTR559_PS_THRES_UP_1);
	if (databuf[1] < 0) {
		APS_DBG("read ps_thres_up_1 fail\n");
		goto out;
	}
	ps_thres_high = databuf[0] + ((databuf[1] & 0x0F) * 256);

	APS_DBG("ps_threshold_high = %d\n", ps_thres_high);

	return ps_thres_high;
...
static int ltr559_ps_enable(struct i2c_client *client, int enable)
{
	...
	ret = ltr559_i2c_write_reg(LTR559_PS_CONTR, regdata);
	if(ret < 0)
	{
		APS_ERR("PS: enable ps ret: %d en: %d \n", ret, enable);
		return ret;
	}
...
static int ltr559_psensor_calibrate(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		printk("PHB: ltr559_ps_read data: %d\n", obj->ps);
		sum += obj->ps;
	}
	...
	ret = ltr559_i2c_write_reg(LTR559_PS_MEAS_RATE, 0x02); 
	if(ret < 0)
	{
		APS_LOG("ltr559 set ps measurement rate error\n");
		return ret;
...
static int ltr559_get_init_noise(void)
{
	...
	//enable ps
	ret = ltr559_ps_enable(ltr559_obj->client, 1);
	if (ret) {
		APS_ERR("ltr559_ps_enable is failed!!\n");
		return ret;
	}
	...
	//disable ps
	ret = ltr559_ps_enable(ltr559_obj->client, 0);
	if (ret) {
		APS_ERR("ltr559_ps_enable is failed!!\n");
		return ret;
	}
...
static int ltr559_als_enable(struct i2c_client *client, int enable)
{
	...
	if (enable == 1) {
		...
		APS_LOG("ALS(0): disable als only \n");
		regdata &= 0xfe;
	}
...
EXIT_ZERO:
	*data = luxdata_int;
	APS_ERR("ltr559 read als data zero!\n");
	return 0;
...
static int ltr559_devinit(void)
{
	...
	res = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 2);
	if(res<0)
	{
...
static int ps_enable_nodata(int en)
{
	...
	ret = ltr559_ps_enable(ltr559_obj->client, en);
	if (ret) {
		APS_ERR("als_enable_nodata is failed!!\n");
		return -1;
	}
...
#endif
#ifdef HQ_PSENSOR_CALIBRATION
	if (regdata & 0x02) {	//ps really enabled
		ret = ltr559_psensor_calibrate();
		if (ret) {
			APS_ERR("ltr559_psensor_calibrate fail!!\n");
			return -1;
		}
	}
#endif
...
/*singal interrupt function add*/
		...
		//}
		printk("PHB: PS Report Interrupt Data: %d\n", value);
