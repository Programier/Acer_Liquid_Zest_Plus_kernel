...
/* eint request */
	if (stk3x1x_obj->irq_node) {
		...
		if (request_irq(stk3x1x_obj->irq, stk3x1x_eint_handler, IRQ_TYPE_LEVEL_LOW, "stk3-eint", NULL)) {
...
static ssize_t stk3x1x_show_status(struct device_driver *ddri, char *buf)
{
	...
	len += scnprintf(buf+len, PAGE_SIZE-len, "Threshold: ps_thd_val_h = %d, ps_thd_val_l = %d\n", atomic_read(&stk3x1x_obj->ps_high_thd_val), atomic_read(&stk3x1x_obj->ps_low_thd_val));
