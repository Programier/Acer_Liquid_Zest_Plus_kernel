...
#if defined(CONFIG_HAPTICS_DRV2604)
static struct DRV2604L_platform_data  drv2604_plat_data = {
	.GpioEnable = 83,			//enable the chip
	.GpioTrigger = 0,			//external trigger pin, (0: internal trigger)
#if defined(CONFIG_HAPTICS_LRA_SEMCO1030)
	//rated = 1.5Vrms, ov=2.1Vrms, f=204hz
	.loop = CLOSE_LOOP,
...
static void vibrator_enable( struct timed_output_dev *dev, int value)
{
	...
	//__gpio_set_value(pDrv2604ldata->PlatData.GpioEnable, 1);
	...
	if(value > 0)
	{
		...
		if((vol_mode > 0x4) && (vol_mode < 0x7)) //0x04=2.5v 0x05=2.8V 0x06=3.0v 0x07=3.3v
		{
			pmic_set_register_value(PMIC_RG_VIBR_VOSEL,vol_mode);			
		}
		else {
			pmic_set_register_value(PMIC_RG_VIBR_VOSEL, 0x5); //default
		}

		value = value >> 4;
		printk("[@@Vinton]vibrator_enable: vibrator after in time = %d,vol_mode=%d\n", value,vol_mode);
...
{
	struct DRV2604L_data *pDrv2604ldata = (struct DRV2604L_data *)filp->private_data;
    printk("%s, dev2604_write start %d \n", __FUNCTION__,buff[0]);	
    ...
    switch(buff[0])
    {	
		 case DRV2604L_TEST_ON:			
			drv2604l_change_mode(pDrv2604ldata, WORK_VIBRATOR, DEV_READY);
			switch_set_state(&pDrv2604ldata->sw_dev, SW_STATE_RTP_PLAYBACK);	       
	     break;
		 
		 case DRV2604L_TEST_OFF:
			 drv2604l_set_go_bit(pDrv2604ldata, STOP);
			 drv2604l_change_mode(pDrv2604ldata, WORK_IDLE, DEV_STANDBY);
			 switch_set_state(&pDrv2604ldata->sw_dev, SW_STATE_IDLE);	 
		 break;
...
static int dev_auto_calibrate(struct DRV2604L_data *pDrv2604ldata)
{
	...
	drv2604l_change_mode(pDrv2604ldata, WORK_CALIBRATION, DEV_STANDBY);
	drv2604l_set_go_bit(pDrv2604ldata, GO);
...
static int drv2604l_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
	...
	if(pDrv2604ldata->PlatData.GpioEnable){
	    ...
	    gpio_direction_output(pDrv2604ldata->PlatData.GpioEnable, 1);
		__gpio_set_value(pDrv2604ldata->PlatData.GpioEnable, 1);
	...
	if(pDrv2604ldata->device_id != DRV2604){
...
static int __init drv2604_subsys_init(void)
{
    printk("%s, drv2604l_init start\n", __FUNCTION__);
...
subsys_initcall(drv2604_subsys_init);
