...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	struct DSI_RX_DATA_REG read_data0;
 	...
 	static int data_paras[2] = {0x0f,0xf0};
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			//DISPCHECK
				printk("[DSI]enter cmp dsi_params->lcm_esd_check_table[%d].cmd == 0x%x\n", i, dsi_params->lcm_esd_check_table[i].cmd);
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {
					...
					if (i!=2) {
						if (data_arr[j] == dsi_params->lcm_esd_check_table[i].para_list[j]) {
							ret = 0; // esd pass
	   				    		...
	   				    		printk("@@##@@ esd cmp fail @@##@@: i = %d, data_arr[j] = 0x%x\n", i, data_arr[j]);
							return 1; // esd fail
						}
					} else {
				...
				if (read_data0.byte1 == dsi_params->lcm_esd_check_table[i].para_list[0]) {
					...
					printk("esd cmp fail: read_data0.byte1 = 0x%x\n", read_data0.byte1);
     					return 1; // esd fail
				}
			}
			
			if (ret == 0)
				printk("@@@ esd cmp success!! @@@\n");
		}
