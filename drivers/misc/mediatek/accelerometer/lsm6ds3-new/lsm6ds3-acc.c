...
int pm_active_enable_flag = 0;
int pm_enable_flag = 0;
extern void pm_acc_enable_data(int enable);
...
static int LSM6DS3_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	struct lsm6ds3_i2c_data *obj = (struct lsm6ds3_i2c_data*)i2c_get_clientdata(client);
	u8 databuf[20];
	int acc[LSM6DS3_ACC_AXES_NUM] = {0};
	int res = 0;
...
//phb modified for take 3s to resume 2016.06.01 start
		...
		if (pm_active_enable_flag == 1) {
			printk("PHB: disable acc by suspend\n");
			pm_enable_flag = 1;
			pm_acc_enable_data(0);
		}
//phb modified for take 3s to resume 2016.06.01 end
...
//phb modified for take 3s to resume 2016.06.01 start
	...
	if (pm_enable_flag == 1) {
		printk("PHB: enable acc by resume\n");
		pm_enable_flag = 0;
		pm_acc_enable_data(1);
	}
//phb modified for take 3s to resume 2016.06.01 end
	atomic_set(&obj->suspend, 0);  
