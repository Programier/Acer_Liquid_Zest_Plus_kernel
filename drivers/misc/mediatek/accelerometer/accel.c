...
extern int pm_active_enable_flag;//phb modified for take 3s to resume 2016.06.01 start
	...
	acc_real_enable(enable);
		if (false == cxt->is_polling_run && cxt->is_batch_enable == false) {
			if (false == cxt->acc_ctl.is_report_input_direct) {
				...
				pm_active_enable_flag = 1;//phb modified for take 3s to resume 2016.06.01
			}
		}
	}
	if (0 == enable) {
		...
		if (true == cxt->is_polling_run) {
			if (false == cxt->acc_ctl.is_report_input_direct) {
				...
				pm_active_enable_flag = 0;//phb modified for take 3s to resume 2016.06.01
			}
		}
	acc_real_enable(enable);
	}
...
void pm_acc_enable_data(int enable)
{
    if (enable == 1)
        acc_enable_data(1);
    else
...
EXPORT_SYMBOL(pm_acc_enable_data);
